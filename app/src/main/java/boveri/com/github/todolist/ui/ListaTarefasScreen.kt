...
fun ListaTarefasContent(
    ...
    tarefaParaDeletarInicial: Tarefa? = null
) {
    var tarefaParaDeletar by remember { mutableStateOf(tarefaParaDeletarInicial) }

    if (tarefaParaDeletar != null) {
        ConfirmacaoExclusaoDialog(
            tarefa = tarefaParaDeletar!!,
            onConfirmar = {
                val t = tarefaParaDeletar!!
...
@Preview(showBackground = true, name = "Lista com diálogo de exclusão")
@Composable
private fun ListaTarefasComDialogoExclusaoPreview() {
    val tarefaExclusao = Tarefa(id = 1, titulo = "Estudar Room", descricao = "Revisar anotações e DAO", concluida = false)
    ListaTarefasContent(
        tarefas = listOf(
            tarefaExclusao,
            Tarefa(id = 2, titulo = "Enviar atividade", descricao = "Upload no portal da FIAP", concluida = true)
        ...
        onCheckedChange = { _, _ -> },
        onDeletar = {},
        tarefaParaDeletarInicial = tarefaExclusao
    )
}
